...
graph LR
    subgraph "External Systems"
        RemoteMgmt["Remote Management<br/>(TR-069/WebPA/WebUI)"]
        ISP["ISP Authentication<br/>Server"]
    end

    subgraph "RDK-B Platform"
        subgraph "RDK-B Middleware"
            TR181Client["TR-181 Clients<br/>(PandM/WebUI/CWMP)"]
            WanMgr["WAN Manager"]
            PSM["PSM Component"]
        ...
        subgraph "System Layer"
            PPPD["pppd Daemon"]
            NetStack["Network Stack<br/>(PPPoE/PPPoA)"]
        end
    end

    RemoteMgmt -->|Configuration| TR181Client
    ISP <-->|PPP Authentication| NetStack
    TR181Client -->|get/set Device.PPP.*<br/>CCSP Message Bus| PppMgr
    PppMgr -->|set Device.X_RDK_WanManager.*<br/>Status Updates| WanMgr
    PppMgr <-->|PSM_Get/Set_Record_Value2<br/>Config Persistence| PSM
    PppMgr <-->|Nanomsg IPC<br/>Session Control & Events| PPPD
    PPPD <-->|Configure/Monitor| NetStack
    ...
    class TR181Client,WanMgr,PSM,PppMgr rdkbComponent;
...
graph TB
    ...
    subgraph "RDK-B Middleware"
        TR181Clients["TR-181 Clients<br/>(PandM/WebUI/CWMP)"]
        WM["WAN Manager"]
        PSM["PSM Component"]
        
        subgraph PppMgr["RdkPppManager"]
            MsgBus["CCSP Message Bus<br/>Interface"]
            DataModel["Device.PPP.*<br/>Data Model Handler"]
            DMLAPI["PPP DML APIs"]
            EventQueue["Event Queue<br/>(POSIX MQ)"]
            IPCClient["IPC Client<br/>(Nanomsg)"]
        end
    end
    ...
    RemoteMgmt -->|Configuration| TR181Clients
    TR181Clients -->|Get/Set Device.PPP.*| MsgBus
    MsgBus <--> DataModel
    DataModel <--> DMLAPI
    DMLAPI -->|Set Device.X_RDK_WanManager.*<br/>Status Updates| WM
    DMLAPI <-->|PSM_Get/Set_Record_Value2<br/>dml.ppp.* namespace| PSM
    DMLAPI -->|Enqueue Start/Stop| EventQueue
    EventQueue --> IPCClient
    IPCClient <-->|Session Control & Events<br/>Nanomsg IPC Socket| PPPD
    DMLAPI -->|sysevent_set| Sysevent
    DMLAPI -->|syscfg_get/set| Syscfg
    PPPD <-->|PAP/CHAP/MS-CHAP| ISP
    ...
    class RemoteMgmt,ISP external;
    class TR181Clients,WM,PSM,PppMgr,MsgBus,DataModel,EventQueue,IPCClient,DMLAPI middleware;
...
* **RDK-B Components**: `CcspPandM` (provides Device.DeviceInfo.UpTime and may act as TR-181 client), `CcspPsm` (configuration persistence), `CcspCommonLibrary` (CCSP framework), `WanManager` (WAN coordination), `CcspCr` (component registry)
* **HAL Dependencies**: `hal_platform` for platform-specific operations (serial number, system info)
...
* **TR-181 Data Model**: RdkPppManager implements complete `Device.PPP.*` object hierarchy per BBF TR-181 Issue 2 specification. Other components access PPP parameters via CCSP Message Bus
...
* **IPC Socket**: Nanomsg socket endpoint defined by `PPP_MANAGER_ADDR` constant from `ipc_msg.h` external header for pppd daemon communication
...
sequenceDiagram
    ...
    IPC->>IPC: nn_bind to PPP_MANAGER_ADDR
...
sequenceDiagram
    ...
    IPC->>IPC: nn_bind PPP_MANAGER_ADDR
...
sequenceDiagram
    ...
    API->>API: PppMgr_StartPppClient
    API->>API: Build pppd command line
    API->>PPPD: system(command) execute pppd
...
- **Parameter Registration**: Parameters registered through `RdkPppManager.xml` configuration file parsed by CCSP Component Registry. DML handler functions mapped to each parameter in XML function definitions
...
| TR-181 Clients (CcspPandM, WebUI, CWMP) | Management clients query and configure PPP interfaces through Device.PPP.* data model. RdkPppManager implements all Device.PPP.* parameters and responds to TR-181 get/set requests | `Device.PPP.Interface.{i}.*` via CCSP Message Bus `CcspBaseIf_getParameterValues()`, `CcspBaseIf_setParameterValues()` |
| WAN Manager | Primary northbound integration - RdkPppManager updates WAN Manager's data model with PPP connection status, IPCP/IPv6CP negotiation results for unified WAN interface coordination | `CcspBaseIf_setParameterValues()` to `Device.X_RDK_WanManager.Interface.{i}.VirtualInterface.{i}.PPP.Status`, `IPCPStatus`, `IPv6CPStatus` via `DmlWanmanagerSetParamValues()` |
| CcspPandM | Limited interaction - RdkPppManager queries system uptime for LastChange calculations. CcspPandM does NOT manage PPP data model | `CcspBaseIf_getParameterValues()` for `Device.DeviceInfo.UpTime` via `DmlPppMgrGetParamValues()` in `GetUptimeinSeconds()` |
| PSM Component | Persistent storage of PPP configuration parameters including Enable, Username, Password, LowerLayers, MaxMRUSize, and interface instance data across reboots | `PSM_Get_Record_Value2()`, `PSM_Set_Record_Value2()` for `dml.ppp.i.{InstanceNumber}.*` namespace parameters |
| **System & Services** |
| pppd Daemon | Southbound interface - PPP session establishment, PAP/CHAP/MS-CHAP authentication, LCP negotiation, IPCP/IPv6CP network control protocol handling, connection monitoring | Nanomsg IPC socket (endpoint defined by `PPP_MANAGER_ADDR` from `ipc_msg.h`) for bidirectional messaging - PppManager sends start/stop commands, receives state events (LCP_UP, IPCP_COMPLETED, INTERFACE_UP, etc.) |
| Sysevent | System-wide event notification for PPP status changes | `sysevent_set()` for `ppp-status`, `wan-status` events (limited usage) |
| Syscfg | System configuration database access for WAN protocol type and UUID generation for DUID | `syscfg_get()` for `wan_proto`, `UUID`; `syscfg_set()` for `UUID` generation |
| Linux Network Stack | Runtime statistics collection and interface monitoring for BytesSent, BytesReceived, PacketsSent, PacketsReceived | `/proc/net/dev`, `/proc/net/dev_extstats` file parsing via `CosaUtilGetIfStats()` |

**Events Published by RdkPppManager:**

| Event Name | Event Topic/Path | Trigger Condition | Subscriber Components |
|------------|-----------------|-------------------|---------------------|
| PPP_Status_Change | `Device.PPP.Interface.{i}.Status` CCSP parameter change notification | PPP interface operational status changes: Up when connection established, Down when terminated, LowerLayerDown when lower layer interface unavailable | WAN Manager, TR-181 Clients (WebUI, CWMP, Remote Management) |
| PPP_ConnectionStatus_Change | `Device.PPP.Interface.{i}.ConnectionStatus` CCSP parameter change notification | PPP connection state transitions: Unconfigured→Connecting→Authenticating→Connected or→Disconnected. Triggered by pppd state events | WAN Manager, TR-181 Clients, Monitoring Services |
| PPP_IPCP_Status_Update | `Device.X_RDK_WanManager.Interface.{i}.VirtualInterface.{i}.PPP.IPCPStatus` setParameterValues to WAN Manager | IPCP negotiation completion or failure. Set to UP when IPv4 address/DNS obtained, DOWN on connection loss | WAN Manager (for unified WAN state machine) |
| PPP_IPv6CP_Status_Update | `Device.X_RDK_WanManager.Interface.{i}.VirtualInterface.{i}.PPP.IPv6CPStatus` setParameterValues to WAN Manager | IPv6CP negotiation completion or failure. Set to UP when IPv6 prefix/address obtained, DOWN on connection loss | WAN Manager (for IPv6 WAN connectivity) |
| PPP_Authentication_Failed | `Device.PPP.Interface.{i}.LastConnectionError` update to ERROR_AUTHENTICATION_FAILURE | ISP rejects PAP/CHAP/MS-CHAP credentials during authentication phase. Received via pppd state event | WAN Manager, Logging/Telemetry Services |
| PPP_Interface_Parameters | `Device.PPP.Interface.{i}.IPCP.*` parameter updates | Dynamic parameter updates: LocalIPAddress, RemoteIPAddress, DNSServers obtained from IPCP negotiation. Set when pppd sends IPCP_COMPLETED event | Internal data model update, CCSP subscribers via value change notifications |
...
sequenceDiagram
    ...
    IPC->>IPC: PppMgr_StartPppClient build command
    IPC->>PPPD: system(command) execute pppd
