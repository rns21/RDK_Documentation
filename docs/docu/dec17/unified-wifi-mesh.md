...
- **AL-SAP Integration (`libalsap`)**: Optional IEEE 1905.1 Abstract Layer Service Access Point library that offloads raw CMDU frame relay to a co-running `ieee1905` daemon over Unix domain sockets; primary transport in OpenWRT deployments
...
unified-wifi-mesh is structured around a shared EM engine library (`src/em`) that is compiled into both the controller and agent binaries. Each binary then registers only its role-specific handlers, command implementations, and orchestration logic through thin adaptor layers (`src/ctrl`, `src/agent`). This split ensures that common IEEE 1905.1 state-machine logic, CMDU message construction/parsing, and DPP cryptography are not duplicated while still permitting the two processes to run with entirely independent lifecycles and address spaces.
...
Data persistence for the controller is provided by MariaDB. The `db_client_t` class manages all database connections. On OpenWRT and Banana Pi R4 builds (gated by `OPENWRT_BUILD` or `_PLATFORM_BANANAPI_R4_`) the MariaDB C client header is included as `<mysql/mysql.h>`; on standard Debian-style Linux it is `<mariadb/mysql.h>`. No syscfg or PSM (Persistent Storage Manager) persistence is used; all persistent topology state lives in MariaDB.
...
| N/A — set unconditionally in OpenWRT makefile                    | N/A            | `OPENWRT_BUILD`                                     | Selects OpenWRT-specific code paths: uses `<mysql/mysql.h>` instead of `<mariadb/mysql.h>` for MariaDB C client, adds `-lstdc++fs` and OpenWRT sysroot library paths                                                                                                                                                                   | Not set — RDK-B/Raspberry Pi builds; set unconditionally in `build/openwrt/agent/makefile` and `build/openwrt/ctrl/makefile` |
| N/A — set in `build/makefile.inc`                                | N/A            | `_PLATFORM_RASPBERRYPI_`                            | Platform identifier for Raspberry Pi cross-compilation; added unconditionally via `CXXFLAGS += -D_PLATFORM_RASPBERRYPI_` in `build/makefile.inc`                                                                                                                                                                                       | Set for all builds using the non-OpenWRT `build/` tree                                                                       |
| N/A                                                              | N/A            | `_PLATFORM_BANANAPI_R4_`                            | Platform identifier for Banana Pi R4; when defined (alongside `OPENWRT_BUILD`) selects `<mysql/mysql.h>` for the MariaDB header include in `db_client.h`                                                                                                                                                                               | Not set by default                                                                                                           |
...
| N/A — set in autoconf `Makefile.am` for ctrl / network_optimiser | N/A            | `UNIT_TEST`                                         | Disables production startup paths in the controller and network optimiser when building test binaries; also set unconditionally in `build/openwrt/ctrl/makefile`                                                                                                                                                                       | Set only in test binary targets (`onewifi_em_ctrl_test`, `onewifi_em_network_optimser`)                                      |
| N/A — passed as `TESTING=1` to make                              | N/A            | `TESTING`                                           | Guards test-specific code blocks in the controller (e.g., in `em_ctrl.cpp` `#ifndef TESTING` skip production main)                                                                                                                                                                                                                     | Not set by default; set when invoking `make test` target in `build/ctrl/makefile` and `build/openwrt/ctrl/makefile`          |
...
- **Build Dependencies**: C++17 compiler, `libcjson`, `libuuid`, `libssl`, `libcrypto` (OpenSSL), `libmariadb` / MariaDB C client, `libpthread`, `libdl`, `libwebconfig`, `libhebus` (HE-BUS), `libalsap` (when `WITH_SAP=1`); on OpenWRT additionally `libstdc++fs`
...
- **Systemd / Init Services**: `em_ctrl` (OpenWRT procd, START=95), `em_agent` (OpenWRT procd, started manually after 30 s delay), `ieee1905_agent` (prerequisite when AL-SAP is enabled)
...
- **Message Bus**: HE-BUS registrations under the service name `EasyMesh_service_agent` (agent) and `tr_181_service` (controller TR-181); R-BUS for TR-181 `Device.WiFi.*` parameter access on RDK-B
- **Configuration Files**: `EasymeshCfg.json` (agent AL-MAC and colocated-mode flag, written to `EM_CFG_FILE` path); TLS certificate and key (`/nvram/test_cert.crt`, `/nvram/test_cert.key`) and `Reset.json` for controller; DPP bootstrapping PEM/connector files under `/nvram/` (`DPPURI.pem`, `C-sign-key.pem`, `net-access-key.pem`, `ppk.pem`, `connector.txt`)
...
sequenceDiagram
    autonumber
    participant Sys as System / procd
...
```mermaid
sequenceDiagram
    participant Boot as boot / procd
...
unified-wifi-mesh decomposes into the following modules. The EM engine (`src/em`) is compiled into both binaries; all other modules are either agent-only or controller-only.
...
- **MariaDB Persistence**: The controller uses `db_client_t` to persist all topology data. The database is initialised by `setup_mysql_db.sh` (config/openwrt/banana-pi/setup_mysql_db.sh) before the controller starts. No PSM or syscfg persistence is used by this component.
