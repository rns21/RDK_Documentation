...
graph TD
    subgraph "External Management"
        ...
        SNMP_NMS[SNMP Network Management]
    end

    subgraph "Protocol Agents"
        TR069PA[TR-069 Protocol Agent]
        WebPA[WebPA Agent]
        SNMP_Agent[SNMP Protocol Agent]
        WiFi_Agent[Wi-Fi Agent]
        Mesh_Agent[Mesh Agent]
    end

    subgraph "RDK-B Middleware"
        NotifyComponent[NotifyComponent]
        Other_Components[Other RDK-B Components<br/>CcspPandM, CcspCr, etc.]
    end

    subgraph "System Layer"
        Message_Queue[POSIX Message Queue<br/>/Notify_queue]
        File_System[File System<br/>/tmp/.NotifyParamListCache]
    end

    %% External connections
    ACS -->|TR-069 Management| TR069PA
    WebUI -->|HTTP/REST| WebPA
    SNMP_NMS -->|SNMP Protocol| SNMP_Agent

    %% Protocol agent registrations
    TR069PA -->|Register Parameters<br/>RBus/DBUS| NotifyComponent
    WebPA -->|Register Parameters<br/>RBus/DBUS| NotifyComponent  
    SNMP_Agent -->|Register Parameters<br/>RBus/DBUS| NotifyComponent
    WiFi_Agent -->|Register Parameters<br/>RBus/DBUS| NotifyComponent
    Mesh_Agent -->|Register Parameters<br/>RBus/DBUS| NotifyComponent

    %% Notification flows
    Other_Components -->|Parameter Changes<br/>Message Queue| NotifyComponent
    NotifyComponent -->|Notifications<br/>RBus/DBUS| TR069PA
    NotifyComponent -->|Notifications<br/>RBus/DBUS| WebPA
    NotifyComponent -->|Notifications<br/>RBus/DBUS| SNMP_Agent
    NotifyComponent -->|Notifications<br/>RBus/DBUS| WiFi_Agent
    NotifyComponent -->|Notifications<br/>RBus/DBUS| Mesh_Agent

    %% System interactions
    NotifyComponent <-->|Message Queue I/O| Message_Queue
    NotifyComponent <-->|Persistent Storage| File_System
    ...
    class ACS,WebUI,SNMP_NMS external;
    class TR069PA,WebPA,SNMP_Agent,WiFi_Agent,Mesh_Agent protocol;
    class NotifyComponent,Other_Components component;
    class Message_Queue,File_System system;
```
...
```mermaid
graph TD
    subgraph "NotifyComponent Container (RDK-B Middleware)"
        subgraph "Registration Manager"
            RegAPI[Registration API Handler]
            ParamRegistry[Parameter Registry<br/>Dynamic Linked List]
            PersistMgr[Persistence Manager<br/>File I/O]
            noteReg["Purpose: Manages protocol agent registrations for TR-181 parameters with persistent storage"]
        end

        subgraph "Notification Engine"
            EventHandler[Event Handler Thread]
            MsgQueue[Message Queue Interface<br/>POSIX mq]
            NotifyDispatcher[Notification Dispatcher]
            NotifyMasks[Notification Mask Manager]
            noteNotify["Purpose: Processes incoming parameter changes and distributes notifications to subscribers"]
        end

        subgraph "IPC Interface Layer"
            RBusInterface[RBus/DBUS Interface]
            TR181Handler[TR-181 Parameter Handler]
            ProtocolComm[Protocol Agent Communication]
            noteIPC["Purpose: Handles communication with protocol agents and RDK-B components"]
        end
    end
    ...
    subgraph "Protocol Agents"
        ...
        SNMPAgent[SNMP Agent]
        WiFiAgent[Wi-Fi Agent]
        MeshAgent[Mesh Agent]
    end

    subgraph "RDK-B Components"
        PandMAgent[P&M Agent]
        CrAgent[Component Registry]
        OtherComps[Other Components]
    end

    subgraph "System Storage"
        TmpStorage["/tmp/.NotifyParamListCache"]
        MsgQueueSys["/Notify_queue"]
    end

    %% Registration flow
    WebPAAgent -->|Register Parameters<br/>RBus| RegAPI
    TR069Agent -->|Register Parameters<br/>RBus| RegAPI
    SNMPAgent -->|Register Parameters<br/>RBus| RegAPI
    WiFiAgent -->|Register Parameters<br/>RBus| RegAPI
    MeshAgent -->|Register Parameters<br/>RBus| RegAPI

    RegAPI --> ParamRegistry
    ParamRegistry --> PersistMgr
    PersistMgr <-->|Read/Write| TmpStorage

    %% Notification flow
    PandMAgent -->|Parameter Changes<br/>POSIX Message Queue| MsgQueue
    CrAgent -->|Parameter Changes<br/>POSIX Message Queue| MsgQueue
    OtherComps -->|Parameter Changes<br/>POSIX Message Queue| MsgQueue

    MsgQueue --> EventHandler
    EventHandler --> ParamRegistry
    EventHandler --> NotifyDispatcher
    NotifyDispatcher --> NotifyMasks
    NotifyMasks --> ProtocolComm

    ProtocolComm -->|Notifications<br/>RBus| WebPAAgent
    ProtocolComm -->|Notifications<br/>RBus| TR069Agent  
    ProtocolComm -->|Notifications<br/>RBus| SNMPAgent
    ProtocolComm -->|Notifications<br/>RBus| WiFiAgent
    ProtocolComm -->|Notifications<br/>RBus| MeshAgent

    %% IPC Layer connections
    RegAPI --> RBusInterface
    ProtocolComm --> RBusInterface
    RBusInterface --> TR181Handler
    EventHandler <--> MsgQueueSys

    classDef component fill:#e1f5fe,stroke:#0277bd,stroke-width:2px;
    classDef external fill:#e8f5e8,stroke:#2e7d32,stroke-width:2px;
    classDef storage fill:#fff3e0,stroke:#ef6c00,stroke-width:2px;

    class RegAPI,ParamRegistry,PersistMgr,EventHandler,MsgQueue,NotifyDispatcher,NotifyMasks,RBusInterface,TR181Handler,ProtocolComm component;
    class WebPAAgent,TR069Agent,SNMPAgent,WiFiAgent,MeshAgent,PandMAgent,CrAgent,OtherComps external;
    class TmpStorage,MsgQueueSys storage;
```
```mermaid
flowchart TD
