...
graph LR
    ...
    subgraph "RDK-B Platform"
        subgraph "Remote Management Agents"
            ProtocolAgents[Protocol Agents TR-069]
        end
        ...
        subgraph "Platform Layer"
            DOCSIS_HAL[DOCSIS HAL]
            EthSw_HAL[Ethernet Switch HAL]
    ...
    HeadEnd -->|DOCSIS Config File/TLV| DOCSIS_HAL
    RemoteMgmt -->|TR-069| ProtocolAgents
    ProtocolAgents -->|IPC| TR069

    TR069 -->|IPC| GwProvApp
    WanMgr -->|IPC| GwProvApp
    PandM -->|IPC| GwProvApp
    OtherComponents -->|IPC| GwProvApp

    GwProvApp <-->|IPC| Sysevent
    GwProvApp <-->|IPC| Syscfg
    GwProvApp <-->|HAL APIs| DOCSIS_HAL
    GwProvApp <-->|HAL APIs| EthSw_HAL
    GwProvApp <-->|HAL APIs| Platform_HAL

    DOCSIS_HAL <-->|Driver Interfaces| Linux
    EthSw_HAL <-->|Driver Interfaces| Linux
    Platform_HAL <-->|Driver Interfaces| Linux

    classDef external fill:#fff3e0,stroke:#ef6c00,stroke-width:2px;
    classDef gwprov fill:#e3f2fd,stroke:#1976d2,stroke-width:3px;
    classDef rdkbComponent fill:#e8f5e8,stroke:#2e7d32,stroke-width:2px;
    classDef system fill:#fce4ec,stroke:#c2185b,stroke-width:2px;

    class HeadEnd,RemoteMgmt external;
    class GwProvApp gwprov;
    class ProtocolAgents,WanMgr,TR069,PandM,OtherComponents rdkbComponent;
    class DOCSIS_HAL,EthSw_HAL,Platform_HAL,Sysevent,Syscfg,Linux system;
```
...
- **AutoWAN Technology Selection**: Automatically detects and configures optimal WAN connectivity method between DOCSIS cable modem and Ethernet WAN interfaces based on link availability and configuration policies when AUTOWAN_ENABLE is defined
...
The north-bound interfaces integrate with RDK-B middleware components through sysevent IPC mechanisms, providing configuration updates, operational status, and event notifications. South-bound integration leverages HAL APIs for hardware control, syscfg for persistent configuration management, and sysevent for real-time system coordination. The design incorporates error handling, recovery mechanisms, and logging to support field deployment requirements and troubleshooting scenarios.

```mermaid
graph LR
    subgraph "External Systems"
        CableHeadEnd[Cable Headend CMTS]
        RemoteMgmt[Remote Management TR-069 Server]
    end

    subgraph "RDK-B Middleware Layer"
        WanMgrComp[WAN Manager]
        TR069Comp[TR-069 Agent]
        PandMComp[PandM]
        NetworkComps[Network Components]
    end

    subgraph "GwProvApp Container"
        subgraph "Main Module"
            StateMachine[Gateway Provisioning State Machine]
        ...
        subgraph "IPC Layer"
            SyseventClient[Sysevent Client]
            SyscfgClient[Syscfg Client]
        end
    end

    subgraph "System Services Layer"
        SyseventDaemon[Sysevent Daemon]
        SyscfgService[Syscfg Service]
        UtopiaServices[Utopia Services]
    end

    subgraph "Hardware Abstraction Layer"
        DOCSIS_HAL[DOCSIS HAL eSafe APIs]
        EthSwitch_HAL[Ethernet Switch HAL]
        Platform_HAL[Platform HAL]
        DHCP_HAL[DHCP v4 Client HAL]
    end

    subgraph "Hardware/OS Layer"
        LinuxKernel[Linux Kernel]
        DOCSISDriver[DOCSIS Driver]
        EthDriver[Ethernet Driver]
    end

    CableHeadEnd -->|TLV Config| DOCSISDriver
    RemoteMgmt <-->|TR-069| TR069Comp

    WanMgrComp <-->|Events| SyseventClient
    TR069Comp <-->|Config Updates| ConfigManager
    PandMComp <-->|Status| EventHandler
    NetworkComps <-->|Events| SyseventClient

    StateMachine --> EventHandler
    StateMachine --> ConfigManager
    StateMachine --> RouterMgr
    StateMachine --> BridgeMgr
    StateMachine --> AutoWANSM
    StateMachine --> TLVParser

    EventHandler --> SyseventClient
    ConfigManager --> SyscfgClient
    TLVParser --> ProvisioningMgr
    AutoWANSM --> LinkMonitor
    AutoWANSM --> ModeSelector
    RouterMgr --> InterfaceMgr
    BridgeMgr --> InterfaceMgr

    SyseventClient --> SyseventDaemon
    SyscfgClient --> SyscfgService
    EventHandler --> UtopiaServices

    InterfaceMgr --> EthSwitch_HAL
    LinkMonitor --> EthSwitch_HAL
    DOCSISEvents --> DOCSIS_HAL
    ProvisioningMgr --> DOCSIS_HAL
    InterfaceMgr --> Platform_HAL
    ConfigManager --> DHCP_HAL

    DOCSIS_HAL --> DOCSISDriver
    EthSwitch_HAL --> EthDriver
    Platform_HAL --> LinuxKernel
    DOCSISDriver --> LinuxKernel
    EthDriver --> LinuxKernel

    classDef external fill:#fff3e0,stroke:#ef6c00,stroke-width:2px;
    classDef middleware fill:#e8f5e8,stroke:#2e7d32,stroke-width:2px;
    classDef mainModule fill:#e3f2fd,stroke:#1976d2,stroke-width:2px;
    ...
    classDef hw fill:#efebe9,stroke:#5d4037,stroke-width:2px;

    class CableHeadEnd,RemoteMgmt external;
    class WanMgrComp,TR069Comp,PandMComp,NetworkComps middleware;
    class StateMachine,EventHandler,ConfigManager mainModule;
    class RouterMgr,BridgeMgr,InterfaceMgr,AutoWANSM,LinkMonitor,ModeSelector,TLVParser,DOCSISEvents,ProvisioningMgr,SyseventClient,SyscfgClient subModule;
    class SyseventDaemon,SyscfgService,UtopiaServices system;
    class DOCSIS_HAL,EthSwitch_HAL,Platform_HAL,DHCP_HAL hal;
    class LinuxKernel,DOCSISDriver,EthDriver hw;
```
...
|------------------|----------------|------------|---------|---------|
| N/A | `bci` | `CISCO_CONFIG_TRUE_STATIC_IP`<br/>`CISCO_CONFIG_DHCPV6_PREFIX_DELEGATION` | Enable Cisco-specific static IP and DHCPv6 prefix delegation configuration support | Disabled |
| N/A | Not `bci` | `FEATURE_SUPPORT_ONBOARD_LOGGING` | Enable onboard logging capability for debug and diagnostics | Enabled when bci is not present |
| N/A | `dslite` | `DSLITE_FEATURE_SUPPORT` | Enable DS-Lite (Dual-Stack Lite) IPv6 transition technology support for IPv4-over-IPv6 tunneling | Disabled |
| N/A | `safec` | Links libsafec library | Enable SafeC bounds-checking library for string operations security | Enabled |
| N/A | Not `safec` | `SAFEC_DUMMY_API` | Use dummy SafeC API stubs when SafeC library is not available | Enabled when safec not present |
| N/A | `lld_support` | `ENABLE_LLD_SUPPORT` | Enable Link Layer Discovery support for eRouter LLD active status monitoring via CM HAL | Disabled |
| `--enable-wanmgr` | `rdkb_wan_manager` | `FEATURE_RDKB_WAN_MANAGER` | Enable WAN Manager integration using generic state machine implementation instead of legacy autowan | Disabled |
| `--enable-unitTestDockerSupport` | N/A | `UNIT_TEST_DOCKER_SUPPORT` | Enable Docker support for unit testing and test build configuration | Disabled |

<br>

**RDK-B Platform and Integration Requirements:**

- **Build Dependencies**: ccsp-common-library, hal-cm, hal-dhcpv4c, hal-ethsw, hal-moca, hal-mso_mgmt, hal-mta, hal-platform, hal-vlan, hal-wifi, ruli, utopia, telemetry
...
- **HAL Dependencies**: Cable Modem HAL (`hal-cm`), Ethernet Switch HAL (`hal-ethsw`), Platform HAL (`hal-platform`), DHCP v4 Client HAL (`hal-dhcpv4c`) for hardware abstraction
...
- **IPC Methods**: Sysevent framework registration using `libsysevent`, syscfg for persistent storage using `libsyscfg`
- **Configuration Files**: `/nvram/syscfg.db` for persistent configuration, `/etc/utopia/` scripts for system integration, TLV configuration file processing
...
- **Worker Threads**:
  - **Sysevent Thread**: Dedicated thread created by `GWP_sysevent_threadfunc()` for sysevent notification handling and system event coordination
  - **AutoWAN Thread**: Created by `WanMngrThread()` when AUTOWAN_ENABLE is defined, monitors WAN interface status and handles automatic WAN mode selection logic
  - **TR-069 Update Thread**: Spawned by `GWP_UpdateTr069CfgThread()` for processing TR-069 configuration updates from DOCSIS TLV parsing
  - **Link State Thread**: Platform-specific thread for physical link status monitoring on Raspberry Pi and embedded platforms
...
- AutoWAN events from link status changes requiring WAN technology switching between DOCSIS and Ethernet interfaces when AUTOWAN_ENABLE is defined
- System restart events from configuration management triggering controlled service restart without full device reboot via `system-restart` sysevent
- Network interface status changes necessitating service coordination through `wan-status`, `lan-status`, `bridge-status` events
...
|-------------|------------|-----------|
| **Gateway Provisioning State Machine** | Core state machine managing device initialization, mode transitions, and operational state coordination through event-driven callbacks | `gw_prov_sm.c`, `gw_prov_sm.h` |
| **Gateway Provisioning Generic State Machine** | WAN Manager-enabled state machine implementation for FEATURE_RDKB_WAN_MANAGER builds providing simplified provisioning flow | `gw_prov_sm_generic.c`, `gw_prov_sm_generic_main.c` |
| **AutoWAN Management** | Automatic WAN technology selection between DOCSIS and Ethernet interfaces based on availability and configuration when AUTOWAN_ENABLE is defined | `autowan.c`, `autowan.h` |
| **TLV Configuration Parser** | Processes DOCSIS configuration files containing Type-Length-Value encoded parameters for TR-069 and system configuration | TLV parsing functions in `gw_prov_sm.c` |
| **System Event Integration** | Handles sysevent framework integration for inter-process communication and system coordination through dedicated event threads | Event handling functions in `gw_prov_sm.c`, `gw_prov_sm_generic.c` |
| **Hardware Abstraction** | Abstracts hardware interface control for DOCSIS, Ethernet switching, and platform-specific operations through HAL API integration | `gw_prov_abstraction.h`, HAL API calls throughout source files |
...
| WiFi/OneWiFi | Network readiness and operational mode notifications | `lan-status`, `bridge-status` sysevent notifications |
| WAN Manager | WAN interface selection and operational mode coordination | `wan-start`, `wan-stop`, `wan-status` sysevent notifications |
| **System & HAL Layers** |
...
| DOCSIS HAL | Cable modem control and status monitoring | `eSafeDevice_Initialize()`, `eSafeDevice_SetErouterOperationMode()` |
...
| `bridge_mode` | `bridge_mode` | Operational mode change to bridge mode | Network stack, firewall, routing services |
...
| `wan-status` | `wan-status` | WAN interface status update | Network services, dependent components |
| `lan-status` | `lan-status` | LAN interface operational status | Network services, bridge manager |
| `ipv4-status` | `ipv4-status` | IPv4 connectivity status change | Network services, firewall |
| `ipv6-status` | `ipv6-status` | IPv6 connectivity status change | Network services, firewall |
...
|---------|---------|-------------------|
| `eSafeDevice_Initialize()` | Initialize DOCSIS eSafe device with MAC address configuration | `gw_prov_sm.c` |
| `eSafeDevice_SetProvisioningStatusProgress()` | Update provisioning state in DOCSIS database for operational tracking | `gw_prov_sm.c` |
| `eSafeDevice_SetErouterOperationMode()` | Configure eRouter operational mode for IPv4/IPv6/dual-stack operation | `gw_prov_sm.c` |
| `getDocsisDbFactoryMode()` | Retrieve DOCSIS factory mode status from cable modem | `gw_prov_sm.c` |
| `CcspHalEthSwGetPortStatus()` | Get Ethernet port link status for AutoWAN link monitoring | `autowan.c` |
| `docsis_IsEnergyDetected()` | Check DOCSIS RF signal presence for AutoWAN mode selection | `autowan.c` |
| `docsis_GetDOCSISInfo()` | Retrieve cable modem status for connection validation | `autowan.c` |
| `parseTlv()` | Parse DOCSIS TLV configuration data from config file | `gw_prov_sm.c` |

### Key Implementation Logic

- **Gateway Provisioning State Machine Engine**: Core state management implemented through event-driven callbacks and state transition functions with error handling and recovery mechanisms. Main state machine logic in `main()` function within `gw_prov_sm.c` for legacy mode and `gw_prov_sm_generic_main.c` for WAN Manager mode with initialization sequence coordination. State transition handlers in callback functions like `GWP_act_DocsisInited_callback()`, `GWP_act_ProvEntry_callback()` for processing DOCSIS events and provisioning states.
  
- **AutoWAN Decision Engine**: Intelligent WAN technology selection based on link availability, configuration priorities, and operational requirements when AUTOWAN_ENABLE is defined. AutoWAN main logic in `WanMngrThread()` function with thread-based monitoring in `autowan.c`. Link status monitoring through `CheckEthWanLinkStatus()` and `CheckWanConnection()` functions for real-time interface state tracking. WAN mode selection algorithms in `TryAltWan()` and `ManageWanModes()` functions for intelligent failover logic.

- **TLV Configuration Processing**: DOCSIS configuration file parsing with validation, parameter extraction, and system configuration updates. TLV parsing initiated in `GWP_act_DocsisCfgfile_callback()` with error handling in `gw_prov_sm.c`. Configuration parameter extraction through `GW_Tr069PaSubTLVParse()` callback function for sub-TLV processing. TR-069 parameter updates handled in dedicated `GWP_UpdateTr069CfgThread()` thread for asynchronous configuration application.

- **System Event Coordination**: Multi-threaded event handling with dedicated threads for different event categories and synchronization. Primary event handling in `GWP_sysevent_threadfunc()` with event registration and processing loop. Event type classification through `Get_GwpThreadType()` function mapping event strings to handler types defined in `gwpthreadMsgArr[]` array. Thread synchronization using pthread mechanisms and sysevent framework coordination for inter-process communication.

- **Error Handling Strategy**: Error detection, logging, and recovery mechanisms for hardware failures, configuration errors, and system resource issues. Hardware error handling with automatic retry mechanisms through state machine re-entry and fallback configurations. Configuration validation with default value restoration through syscfg database and corruption detection via TLV parsing status checks. System resource monitoring with graceful degradation and error reporting through telemetry using `t2_event_d()` function calls.

- **Logging & Debugging**: Multi-level logging with RDK Logger integration, telemetry event generation, and debug configuration support. Conditional logging through `GWPROV_PRINT()` macro with RDK Logger integration when `FEATURE_SUPPORT_RDKLOG` is defined. Telemetry event generation using `t2_event_d()` for operational metrics and error conditions in critical code paths. Debug configuration support through `/etc/debug.ini` file parsing and runtime log level control via `RDK_LOG1()` macro.
...
| `/nvram/syscfg.db` | Persistent system configuration database | syscfg CLI commands, configuration reset |
| `/etc/utopia/` | Utopia service scripts for system integration | Platform-specific script customization |
| `/etc/debug.ini` | Debug configuration for log levels and verbosity | Manual editing, debug flag modification |
| `/nvram/ETHWAN_ENABLE` | AutoWAN Ethernet WAN enable flag (Intel Puma7) | System configuration, AutoWAN logic |
| `/var/tmp/eSafeCfgFile.downloaded` | Downloaded eSafe configuration file (Intel Puma7) | DOCSIS subsystem download |
