...
graph TB
    ...
    IPCClient <-->|Session Control & Events<br/>ipc:///tmp/ppp_manager| PPPD
...
| **System & Services** |
| pppd Daemon | Southbound interface - PPP session establishment, PAP/CHAP/MS-CHAP authentication, LCP negotiation, IPCP/IPv6CP network control protocol handling, connection monitoring | Nanomsg IPC socket `ipc:///tmp/ppp_manager` for bidirectional messaging - PppManager sends start/stop commands, receives state events (LCP_UP, IPCP_COMPLETED, INTERFACE_UP, etc.) |
