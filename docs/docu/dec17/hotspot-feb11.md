...
graph TD
    subgraph "Container: CCSP Hotspot (C, Autotools)"
        ...
        subgraph "API Library (libHotspotApi.so)"
            ...
            WC["☁️ WebConfig Handler<br/>(HotspotJansson.c)"]
...
sequenceDiagram
    ...
    Note over TM: Create shared memory segment<br/>for statistics (key: 765889)
...
sequenceDiagram
    ...
    alt Hotspot Enabled
        ...
        Note over HS: Create shared mem (key: 765889)
...
| Shared Memory | Store keep-alive statistics for access by diagnostic tools and monitoring scripts without IPC overhead | System V shared memory: `shmget(key=765889, size=sizeof(hotspotfd_statistics_s), IPC_CREAT)`, `shmat()`, Structure: `hotspotfd_statistics_s` with counters and status flags |
...
| `shmget() / shmat() / shmdt()` | Create and access System V shared memory segment for keep-alive statistics; allows inter-process communication without message passing overhead | `hotspotfd.c` (hotspotfd_setupSharedMemory(), kKeepAlive_Statistics key 765889) |
...
- **Tunnel Lifecycle State Machine**: The tunnel manager maintains a finite state machine with states: **UNINITIALIZED** (before first configuration), **DISABLED** (hotspot disabled via config), **INITIALIZING** (creating GRE interface and VLANs), **PRIMARY_ACTIVE** (operating on primary endpoint), **SECONDARY_ACTIVE** (failed over to secondary endpoint), **WAN_DOWN** (paused due to WAN outage), and **ERROR** (unrecoverable failure). State transitions are triggered by configuration changes, keep-alive failures, WAN status events, and endpoint recovery. 
  
  - Main implementation in [hotspotfd.c](hotspot/source/hotspotfd/hotspotfd.c) (lines 100-2439: main keep-alive loop, tunnel creation, state management)
  - State transition handlers in [hotspotfd.c](hotspot/source/hotspotfd/hotspotfd.c) (lines 400-600: endpoint switching logic, 1200-1400: WAN status handling)
...
- **Keep-Alive Health Monitoring Algorithm**: Implements a threshold-based failure detection mechanism with the following logic:
  ...
  - Implementation in [hotspotfd.c](hotspot/source/hotspotfd/hotspotfd.c) (lines 1600-2200: keep-alive loop and ICMP packet handling)
  - Checksum validation: `hotspotfd_checksum()` function (lines 600-650)
