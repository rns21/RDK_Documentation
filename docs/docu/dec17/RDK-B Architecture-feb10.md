...
![RDK-B Architecture](./RDK-B_Architecture.png)

### Application Layer

The Application Layer serves as the primary interface through which CPE consumers interact with the device, including support for third-party applications. This layer hosts web-based applications that provide browser-accessible interfaces for device configuration and monitoring, including local web UI capabilities for customer self-service with responsive designs optimized for both desktop and mobile access. It also supports operator-specific applications for network management, diagnostics, and CPE provisioning, as well as third-party applications that extend device functionality with capabilities such as parental controls, security services, and smart home integration.
.
### Middleware Layer
...
*   **Generic Open Source Components**: Standard Linux networking utilities integrated into the middleware to support specific RDK-B use cases, including IP Tables for firewall rule management, IGMP Proxy for multicast forwarding, UPnP IGD for port mapping, and Bridge Utils for Ethernet bridging.
...
*   **Management Protocols and Device Services**: Protocol agents that enable remote device management using mechanisms such as TR-369, TR-069, WebPA, Webconfig, and SNMP, along with device services including SSH access, local WebUI, firmware upgrades, Telemetry 2.0, Remote Feature Control, log and crash upload, and log rotation to support device lifecycle management and diagnostics.
