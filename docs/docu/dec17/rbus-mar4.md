...
The following CMake options (source: CMakeLists.txt) control the main components and features built in RBus. All options are set via `-D<OPTION>=ON|OFF` during CMake configuration.

| Build Option | Purpose | Default | Key Components |
|-------------|---------|---------|----------------|
| `BUILD_RBUS_DAEMON` | Build rtrouted message broker | `ON` | `rtrouted`, `rtrouted_diag` |
| `BUILD_RBUS_SAMPLE_APPS` | Build example applications | `ON` | Provider/Consumer samples |
| `BUILD_RBUS_TEST_APPS` | Build test applications | `ON` | Test suites, benchmarks |
| `BUILD_ONLY_RTMESSAGE` | Build only message layer | `OFF` | `rtMessage` library only |
| `ENABLE_RDKLOGGER` | Enable RDK logging system | `OFF` | RDK logging integration |

**Usage Example:**
```bash
# Standard build with daemon and samples
cmake -DBUILD_RBUS_DAEMON=ON -DBUILD_RBUS_SAMPLE_APPS=ON ..

# Build only rtMessage transport layer
cmake -DBUILD_ONLY_RTMESSAGE=ON ..

# Production build with RDK logging
cmake -DBUILD_RBUS_DAEMON=ON -DENABLE_RDKLOGGER=ON ..
```
...
RBus implements a hierarchical data model following TR-069 conventions with four primary element types managed through the `rbusDataElement_t` structure:

| Element Type | Description | Example Names | Handler Functions |
|-------------|-------------|---------------|-------------------|
| `RBUS_ELEMENT_TYPE_PROPERTY` | Readable/writable parameters | `Device.WiFi.Radio.1.Enable` | `rbusGetHandler_t`, `rbusSetHandler_t` |
| `RBUS_ELEMENT_TYPE_TABLE` | Multi-instance objects | `Device.WiFi.Radio.{i}` | `rbusTableAddRowHandler_t`, `rbusTableRemoveRowHandler_t` |
| `RBUS_ELEMENT_TYPE_EVENT` | Asynchronous notifications | `Device.WiFi.StatusChange!` | `rbusEventSubHandler_t` |
| `RBUS_ELEMENT_TYPE_METHOD` | Remote procedure calls | `Device.Reboot()` | `rbusMethodHandler_t` |

> **Source:** include/rbus.h, src/rbus/rbus_element.c, src/rbus/rbus.c

### Data Model Element Registration

![Element Types and Registration Process](./rbus-element-registration.png)
