...
The component operates as a scheduled data collection agent that periodically polls WiFi HAL APIs to retrieve device association information, radio traffic metrics, and neighboring AP scan results. Harvester packages this telemetry data into structured Avro-encoded messages and transmits them to cloud-based data processing systems through the WebPA messaging infrastructure. The component supports configurable polling and reporting intervals for each telemetry report type with persistent storage of configuration parameters through PSM integration.
...
graph TD
    subgraph Harvester ["Harvester Component"]
        ...
        subgraph CollectionModules ["Telemetry Collection Modules"]
            IDWModule[Interface Devices WiFi]
            RISModule[Radio Interface Statistics]
            NAPModule[Neighboring AP Scanner]
        end
...
| N/A | `seshat` | `ENABLE_SESHAT` | Enable Seshat integration for secure token management in WebPA communication | Disabled |
...
* **RDK-B Components**: `CcspPsm`, `CcspWiFiAgent` (or OneWifi), `CcspWebPA`
* **HAL Dependencies**: WiFi HAL APIs for device association queries, radio statistics, and neighboring AP scans
* **Systemd Services**: PSM services must be active before harvester initialization for configuration parameter retrieval
...
* **Startup Order**: Initialize after PSM, WiFi agent, and WebPA services are running to ensure configuration retrieval and message transmission capabilities
...
- **Worker Threads**: 
  - **Interface Devices WiFi Thread**: Polls associated device information at configured intervals and triggers Avro encoding and transmission
  - **Radio Interface Statistics Thread**: Collects radio-level performance metrics periodically and packages data for upstream reporting
  - **Neighboring AP Thread**: Performs WiFi channel scans to detect neighboring access points and generates telemetry reports
  - **Parodus Handler Thread**: Maintains WebPA/Parodus connection and handles upstream message transmission with retry logic
...
Harvester follows a structured initialization sequence establishing message bus connectivity, loading persisted configuration from PSM, and spawning worker threads for telemetry collection based on enabled report types.
...
During operation, Harvester responds to configuration changes, network events, and WebPA connectivity status affecting telemetry collection behavior.
...
- Override TTL expiration causing automatic reversion to default configuration parameters retrieved from PSM
- WebPA connection loss triggering buffering of telemetry messages and retry attempts with exponential backoff
...
- Transmission context switches between immediate send and retry queue when WebPA/Parodus connection availability changes
...
|-------------|------------|-----------|
| **Interface Devices WiFi Module** | Collects detailed information about WiFi clients associated with each SSID by polling WiFi HAL or RBus APIs including device MAC addresses, signal strength, PHY rates, operating modes, and connection timestamps | `harvester_associated_devices.c`, `harvester_associated_devices.h`, `harvester_associated_devices_avropack.c` |
| **Radio Interface Statistics Module** | Gathers radio-level performance metrics including channel utilization, noise floor, transmission counts, retry statistics, and error rates for capacity analysis and network health monitoring | `harvester_radio_traffic.c`, `harvester_radio_traffic.h`, `harvester_radio_traffic_avropack.c` |
| **Neighboring AP Module** | Performs periodic WiFi channel scans to detect surrounding access points and collects SSID, BSSID, channel number, signal strength, security settings, and operating standards for spectrum analysis | `harvester_neighboring_ap.c`, `harvester_neighboring_ap.h`, `harvester_neighboring_ap_avropack.c` |
...
| PSM | Configuration parameter persistence and retrieval for polling periods, reporting intervals, enable flags, and default values | `PSM_Get_Record_Value2()`, `PSM_Set_Record_Value2()` |
| WebPA/Parodus | Upstream telemetry message transmission to cloud analytics using WRP protocol with Avro-encoded binary payloads | `libpd_send()`, `sendWebpaMsg()` |
| WiFi Agent | Indirect interaction through WiFi HAL for device association queries and radio statistics when not using OneWifi RBus mode | WiFi HAL function invocations |
| **System & HAL Layers** |
...
|------------|-----------------|-------------------|---------------------|
| Telemetry Report | WebPA WRP Message | Reporting period expiration with collected data ready for transmission | WebPA/Parodus forwards to cloud |

### IPC Flow Patterns

**Primary IPC Flow - Telemetry Collection:**

```mermaid
sequenceDiagram
    participant Harvester as Harvester Thread
    ...
    Harvester->>Avro: Encode Telemetry Data
    Avro-->>Harvester: Avro Binary Payload
    Harvester->>WebPA: sendWebpaMsg(payload)
    WebPA->>Cloud: WRP Message (Avro/Binary)
    Cloud-->>WebPA: ACK
    WebPA-->>Harvester: Success
```
