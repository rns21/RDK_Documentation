...
- **Build Dependencies**: C/C++ toolchain with autotools and pkgconfig (`inherit autotools pkgconfig`); build-time recipe dependencies: `virtual/vendor-media-utils` (RMF AudioCapture provider), `media-utils-headers` (supplies `rmfAudioCapture.h`), `iarmbus`, `iarmmgrs`, `libunpriv`, `safec-common-wrapper`, and `safec` (conditional on `safec` in `DISTRO_FEATURES`); runtime dependency on `virtual/vendor-media-utils`; link flags: `-lprivilege`, and conditionally `pkg-config --libs libsafec` when `safec` is in `DISTRO_FEATURES`; compile flags: `-DDROP_ROOT_PRIV` (enables privilege-drop path at build time), conditionally `pkg-config --cflags libsafec` or `-fPIC`, and `-DSAFEC_DUMMY_API` when `safec` is absent from `DISTRO_FEATURES`.
- **Device Services / HAL**: RMF AudioCapture HAL (`rmfAudioCapture.h`) — full API surface: `RMF_AudioCapture_Open()` (open primary source), `RMF_AudioCapture_Open_Type()` (open primary or auxiliary source by `RMF_AudioCaptureType`), `RMF_AudioCapture_GetStatus()` (query started/format/freq/overflow state via `RMF_AudioCapture_Status`), `RMF_AudioCapture_GetDefaultSettings()` (retrieve baseline `RMF_AudioCapture_Settings` before start), `RMF_AudioCapture_GetCurrentSettings()` (retrieve settings currently in effect on a started handle), `RMF_AudioCapture_Start()` (apply settings and begin PCM delivery via `RMF_AudioCaptureBufferReadyCb`), `RMF_AudioCapture_Stop()` (halt delivery), `RMF_AudioCapture_Close()` (release handle and hardware resources); return type `rmf_Error` with values `RMF_SUCCESS`, `RMF_ERROR`, `RMF_INVALID_PARM`, `RMF_INVALID_HANDLE`, `RMF_NOT_INITIALIZED`, `RMF_INVALID_STATE`.
