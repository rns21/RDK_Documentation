...
At the device level, this component enforces access restrictions on RDK daemons and services, including platform processes (`IARMDaemonMain`, `dsMgrMain`, `pwrMgrMain`), middleware services (`tr69hostif`, `parodus`, `webconfig`), and the WPEFramework processes (`WPEFramework`, `WPEProcess`). Each process is confined to the files, capabilities, and kernel objects explicitly listed in its profile. A profile operates in either `enforce` mode (violations are denied and logged by the kernel) or `complain` mode (violations are logged but not denied). An operator-writable blocklist at `/opt/secure/Apparmor_blocklist` allows per-process mode override at runtime without reflashing.
...
The tests below are performed on a target device. The CI violation scan runs automatically on every pull request and does not require manual execution.
