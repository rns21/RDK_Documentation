...
- **Device Services / HAL**: V4L2 M2M video decoder device, accessible at `/dev/video10` by default (configurable via `device` property). The platform HAL must expose a compliant V4L2 M2M interface supporting `VIDIOC_QUERYCAP`, `VIDIOC_ENUM_FMT`, `VIDIOC_S_FMT`, `VIDIOC_REQBUFS`, `VIDIOC_QBUF`, and `VIDIOC_DQBUF`.
...
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Wayland Compositor**        |                                                                                                                     |                                                                                                                                                                      |
| Westeros Compositor           | Surface creation, geometry, visibility, z-order, opacity control                                                    | `wl_compositor_create_surface()`, `wl_simple_shell_set_geometry()`, `wl_simple_shell_set_visible()`, `wl_simple_shell_set_zorder()`, `wl_simple_shell_set_opacity()` |
| Westeros Compositor           | Video path and transform change notifications                                                                       | `wl_vpc_surface_set_geometry()`, `vpcVideoPathChange`, `vpcVideoXformChange`                                                                                         |
| Westeros Compositor           | Decoded frame submission via simplebuffer protocol                                                                  | `wl_sb` (simplebuffer Wayland extension)                                                                                                                             |
| **Device Services / HAL**     |                                                                                                                     |                                                                                                                                                                      |
