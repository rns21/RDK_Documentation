...
Membership information is maintained entirely in the daemon's in-memory `m_registeredList` (a GLib `GList`), and resource ownership is tracked in the `m_resourceOwner[]` array. Both structures are initialized fresh on each daemon startup. Logs are written to `/var/volatile/log/iarmbusd.log` as configured in the systemd service unit.
...
```mermaid
sequenceDiagram
    participant ClientApp as Middleware Client
