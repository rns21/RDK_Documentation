...
| -------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| **Plugins**                      |                                                                                                                                   |                                                                                            |
| `WPEFramework DeviceInfo`        | Retrieve device identity (device ID, device type, partner ID, experience) at startup                                              | `ctrlm_thunder_plugin_device_info_t`                                                       |
| `WPEFramework System`            | Monitor firmware image information and system readiness notifications                                                             | `ctrlm_thunder_plugin_system_t`                                                            |
| `WPEFramework PowerManager`      | Query current power state and receive power state change events                                                                   | `ctrlm_thunder_plugin_powermanager_t`, `ctrlm_thunder_powermanager.cpp`                    |
| `WPEFramework SystemAudioPlayer` | Play chime audio files (open chime, close chime, privacy chime) on voice session events                                           | `ctrlm_thunder_plugin_system_audio_player_t`                                               |
| **Device Services /HAL**         |                                                                                                                                   |                                                                                            |
...
- **Logging & Diagnostics**: Uses the `rdkx_logger` framework. The module identifier is `XLOG_MODULE_ID_CTRLM`. Log output includes `XLOGD_INFO`, `XLOGD_ERROR`, and `XLOGD_FATAL` levels. PII masking (key code values) is enabled for production builds via the `mask_pii` configuration field.
