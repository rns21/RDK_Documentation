...
In the whole RDK-E architecture. Explain the components in high level only w.r.t. to the RDK-E architecture layer ( application layer, firebolt layer, thunder layer, rdk core layer(rdk shell/app manager/westeros,graphics, aamp etc. ))
...
[This can be a mermaid sequence diagram kind of one  ]
```
