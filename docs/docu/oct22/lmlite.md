...
| CcspPandM | TR-181 parameter registration and access control, configuration management | DBus/RBus | CCSP Message Protocol/JSON | Request-Response/Event Subscription | `registerCapabilities`, `getParameterValues`, `setParameterValues` |
| CcspWiFiAgent | WiFi device association/disassociation events, wireless client enumeration | DBus Events/Direct API | CCSP Events/JSON | Event-Driven/Periodic Polling | `Device.WiFi.AccessPoint.{i}.AssociatedDevice.{i}.*`, WiFi HAL callbacks |
| CcspMoCAAgent | MoCA network topology changes, node status updates, device discovery | DBus Notifications | CCSP Parameters | Event Subscription/Polling | `Device.MoCA.Interface.{i}.AssociatedDevice.{i}.*` |
...
| Device_Presence | `Device.Hosts.Host.{i}.ActiveChange` | Device online/offline status change | JSON: `{mac, active, last_seen, confidence}` | Parental Control, WiFi Agent, WebPA |
...
|-------------|-----------------|---------|------------------|------------------|
| CcspWiFiAgent | `Device.WiFi.AccessPoint.AssociatedDeviceNumberChange` | WiFi client association/disassociation detection | JSON: `{interface, mac, associated, signal_strength}` | `wifi_device_event_handler()` |
...
| SystemD Network | `NetworkManager.DeviceAdded/Removed` | Network interface up/down events | DBus: Interface properties | `network_interface_event_handler()` |
...
sequenceDiagram
    ...
    WiFiAgent->>LMLite: Device Association Event (DBus)
    ...
    LMLite-->>WiFiAgent: Event Processing Complete
```
