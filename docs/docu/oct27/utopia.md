...
flowchart TD
    ...
    class WiFiAgent,WANManager,EthAgent,MoCAAgent network;
    class Utopia utopia;
...
| CcspPandM | Configuration parameter management and TR-181 data model synchronization | DBus Method Calls | DBus XML Messages | Synchronous Request-Response | `GetParameterValues()`, `SetParameterValues()`, `AddObject()` |
| CcspPsm | Persistent storage manager integration for cross-component data sharing | DBus/Shared Memory | Binary/XML Structures | Asynchronous Updates | `PsmSet()`, `PsmGet()`, `PsmGetRecordKeys()` |
| TR-069 Protocol Agent | Remote management parameter synchronization and firmware management | RBus/DBus | JSON/XML Payloads | Event-Driven | `DeviceInfo.`, `Device.Ethernet.`, `Device.WiFi.` |
| Component Registrar | Service discovery and component lifecycle management | DBus Service Bus | DBus Introspection XML | Service Registration | `RegisterComponent()`, `UnregisterComponent()` |
| **Network Management Components** |
| WiFi Agent | WiFi interface configuration, status monitoring, and event coordination | Sysevent Bus | Key-Value Events | Publish-Subscribe | `wifi-status`, `wifi-restart`, `wlan-configuration` |
...
| SystemD Services | Process lifecycle management and service coordination | DBus System Bus | SystemD Unit Control | Service Management | `systemctl start/stop/restart`, service dependency management |
