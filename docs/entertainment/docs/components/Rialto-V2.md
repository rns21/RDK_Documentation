...
At its core, Rialto consists of three main components working together to deliver a comprehensive media solution:
...
**Rialto GStreamer Plugin** - Integration layer that bridges GStreamer-based applications with the Rialto media server. This plugin provides custom GStreamer sink elements (rialtomseaudiosink, rialtomsevideosink, rialtomsesubtitlesink, rialtowebaudiosink) that enable seamless integration with existing GStreamer pipelines in web browsers and native applications.

**Rialto OCDM** - OpenCDM (Content Decryption Module) adapter enabling DRM (Digital Rights Management) functionality through the Rialto framework. This component implements the standard OpenCDM interface expected by EME (Encrypted Media Extensions) implementations, routing all DRM operations through Rialto's MediaKeys service for centralized license management and secure content decryption.

Rialto enables multiple applications to share hardware media decoders efficiently while maintaining process isolation. The Server Manager orchestrates multiple RialtoSessionServer instances, each dedicated to a specific application, with each session assigned a unique Unix domain socket for IPC communication. The design provides scalability, fault isolation, and optimized resource utilization across the RDK-V middleware stack. By isolating media processing in separate processes, application crashes do not affect media playback, and system resources can be managed more effectively based on application priority and state.

### Rialto Framework Introduction

The following diagram provides a comprehensive overview of the Rialto media framework architecture, illustrating the relationships between client applications, the Rialto framework components, and the underlying hardware abstraction layers.

![Rialto Introduction](../../images/RialtoOverviewFig.png)
- This diagram shows the complete Rialto architecture including:
 ...
 The diagram illustrates the complete data flow from application-level media APIs through the Rialto framework to hardware decoders, showing IPC communication paths, process boundaries, and technology stack at each layer.
...
- **GStreamer Integration**: Provides custom GStreamer sink elements (rialtomsevideosink, rialtomseaudiosink, rialtomsesubtitlesink, rialtowebaudiosink) enabling seamless integration with GStreamer-based applications. Implements Media Source Extensions (MSE) data flow model with both pull and push mode support for optimal buffer management.

- **DRM Support**: OpenCDM adapter (rialto-ocdm) enables encrypted content playback with support for multiple DRM systems (Widevine, PlayReady, ClearKey) through the EME (Encrypted Media Extensions) API. Provides centralized DRM session management with hardware-backed decryption and secure key storage.

- **Protobuf-based IPC**: High-performance communication using Protocol Buffers over Unix domain sockets, supporting RPC calls, asynchronous events, and file descriptor passing for zero-copy buffer sharing. Strongly-typed message definitions ensure protocol compatibility across versions.

- **Media Pipeline Services**: Comprehensive APIs for media source attachment, playback control (play/pause/seek/stop), rendering, volume control, and playback state management. Supports multiple simultaneous audio and video sources with synchronization and timing control.
...
The framework operates in three distinct layers. At the top, client libraries (embedded in applications through GStreamer plugins or OCDM adapters) provide standard media APIs that applications already understand. In the middle, the Server Manager acts as a process orchestrator, spawning and managing RialtoSessionServer instances based on application lifecycle and resource availability. At the bottom, each session server contains the actual media processing logic built on GStreamer, interfacing with platform-specific HAL implementations for hardware acceleration.
...
- **DISTRO Features**: `DISTRO_FEATURES += "rdk-v"` must be enabled in Yocto build configuration for Rialto integration. This feature flag enables the necessary RDK-V middleware components and configurations required for Rialto to function properly.

- **Build Dependencies**: 
  - Protocol Buffers (protobuf) >= 3.6 for IPC message serialization and RPC stub generation
  - GStreamer >= 1.14 including gst-plugins-base, gst-plugins-good for pipeline construction
  - jsoncpp library for configuration file parsing in Server Manager
  - CMake >= 3.10 for build system orchestration
  - C++17 compliant compiler (GCC >= 7.0 or Clang >= 5.0) for modern C++ features
  - Google Test framework for unit test execution (optional, development only)
...
- **Systemd Services**: 
  - `RialtoServerManager.service` must be configured to start during system boot (Type=notify)
  - DBus system bus must be available for inter-component communication
  - Proper service dependencies configured: network.target, dbus.service
...
- **Configuration Files**: 
  ...
  - Application-specific socket paths in `/tmp/rialto-*` (created dynamically)
...
- **Startup Order**: 
  ...
  - Graphics compositor must be initialized before video rendering capabilities are used
  - DRM system must be available before encrypted content playback
  - Thunder framework should be started before RialtoServerManager plugin activation
...
![state diagram](../../images/RialtoClientPlayerSessionStateMachine.png) - This diagram illustrates:
...
**Yocto Integration**:
- Recipe: `meta-rdk-video/recipes-extended/rialto/rialto_git.bb`
- Dependencies: `gstreamer1.0`, `protobuf`, `jsoncpp`, `opencdm`
- PACKAGECONFIG options: `servermanager`, `client`, `gstreamer-plugin`, `ocdm`
- Install paths: `/usr/lib/` (libraries), `/usr/bin/` (executables), `/etc/rialto/` (configuration)

---

**Rialto Media Framework** provides a robust, scalable, and secure solution for media playback in RDK-V platforms. Its client-server architecture with process isolation, protobuf-based IPC, and comprehensive HAL integration enables efficient hardware resource sharing while maintaining stability and security. The framework's modular design supports multiple applications, DRM systems, and platform variants, making it a versatile component in the RDK-V middleware stack.

For more information:
- Project Documentation: https://rdkcentral.github.io/rialto/
- Source Code: https://github.com/rdkcentral/rialto
- RDK Wiki: https://wiki.rdkcentral.com/display/ASP/Rialto
