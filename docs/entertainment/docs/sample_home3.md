---
hide:
  - navigation
  - toc
---
#
<div class="ent-home">
  <!-- Image Banner -->
  <section class="img-banner">
    <!-- <img src="../../../assets/remote.jpg" alt="Banner Image" class="banner-image" /> -->
    <div class="banner-text">
      <h1 class="hd-semibold">Getting started with RDK for Entertainment</h1>
      <!-- <p>Build Apps, manage media and UI layers in a video platform</p> -->
    </div>
  </section>
  <!-- Boxes section -->
  <section class="expandable-section">
    <!-- Box 1 -->
    <div class="expandable-wrap">
    <div class="expandable-box" data-box="1">
      <div class="box-main">
        <img src="../../../assets/icons/rocket (1).png" class="box-header-icon"> 
        <div class="box-header">
          <h3>Getting Started</h3>
        </div>
        <p class="box-description">If you’re new to RDK, start by getting an overview of the platform, explore ways to try out RDK and learn how to build apps in RDK</p>
        <span class="expand-icon material-icons" style="background-color:var(--rdk-dark-blue);">add</span>
      </div>
      <!-- Box 1 sub boxes -->
      <div class="sub-boxes hidden">
        <div class="sub-box">
          <div class="sub-header">
            <h4>Overview</h4>
          </div>
          <p>For an overview of RDK middleware, please follow below link.</p>
          <a href="" class="view-docs-btn">View Docs</a>
          <span class="sub-icon material-icons">description</span>
        </div>
        <div class="sub-box">
          <div class="sub-header">
            <span class="sub-icon material-icons">preview</span>
            <h4>Preview RDK</h4>
          </div>
          <p>To get an experience of RDK, please follow the below link.</p>
          <a href="../../../preview-rdk/docs/getting-started/" class="view-docs-btn">View Docs</a>
        </div>
        <div class="sub-box">
          <div class="sub-header">
            <span class="sub-icon material-icons">try</span>
            <h4>Try Out RDK</h4>
          </div>
          <p>For a hands-on experience in real hardware, please follow the below link.</p>
          <a href="../tryout_rdkv_rdk7/" class="view-docs-btn">View Docs</a>
        </div>
        <div class="sub-box">
          <div class="sub-header">
            <span class="sub-icon material-icons">apps</span>
            <h4>Releases</h4>
          </div>
          <p>Know more about the RDK-E releases available to the community</p>
          <a href="https://wiki.rdkcentral.com/spaces/CMF/pages/414065624/RDK-E+Video+Code+Releases" class="view-docs-btn">View Docs</a>
        </div>
      </div>
      <span class="expandable-icon-bg material-icons" >flight</span>
    </div>
    </div>
    <!-- Box 2 -->
    <div class="expandable-wrap">
    <div class="expandable-box" data-box="2">
      <div class="box-main">
        <img src="../../../assets/icons/application.png" class="box-header-icon"> 
        <div class="box-header">
          <h3>App Ecosystem</h3>
        </div>
        <p class="box-description">Learn how to develop and integrate apps, and discover the frameworks supported by RDK for app development</p>
        <span class="expand-icon material-icons" style="background-color:var(--rdk-blue);">add</span>
      </div>
    <!-- Box 2 sub boxes -->
      <div class="sub-boxes hidden">
        <div class="sub-box">
          <div class="sub-header">
            <span class="sub-icon material-icons">polyline</span>
            <h4>Overview</h4>
          </div>
          <p>For an overview of App ecosystem support across RDK, please follow the below link.</p>
          <a href="" class="view-docs-btn">View Docs</a>
        </div>
        <div class="sub-box">
          <div class="sub-header">
            <span class="sub-icon material-icons">badge</span>
            <h4>Build an App</h4>
          </div>
          <p>To Explore how to build and run an app in RDK, please follow the below link</p>
          <a href="" class="view-docs-btn">View Docs</a>
        </div>
        <div class="sub-box">
          <div class="sub-header">
            <span class="sub-icon material-icons">devices</span>
            <h4>Firebolt</h4>
          </div>
          <p>Learn more about Firebolt, the RDK Application Framework SDK</p>
          <a href="https://rdkcentral.github.io/firebolt/apis/latest/" class="view-docs-btn">View Docs</a>
        </div>
        <div class="sub-box">
          <div class="sub-header">
            <span class="sub-icon material-icons">smartphone</span>
            <h4>Lightning</h4>
          </div>
          <p>Learn more about Lightning, the Javascript-based lightweight app development language</p>
          <a href="https://www.lightningjs.io/" class="view-docs-btn">View Docs</a>
        </div>
      </div>
      <span class="expandable-icon-bg material-icons"style="color:var(--rdk-blue);">devices</span>
    </div>
    </div>
    <!-- Box 3 -->
    <div class="expandable-wrap">
    <div class="expandable-box" data-box="3">
      <div class="box-main">
        <img src="../../../assets/icons/system-integration.png" class="box-header-icon"> 
        <div class="box-header">
          <h3>Integration</h3>
        </div>
        <p class="box-description">If you are a platform vendor, OEM, or operator, learn how to integrate RDK into your ecosystem</p>
        <span class="expand-icon material-icons"  style="background-color:var(--rdk-green);">add</span>
      </div>
      <!-- Box 3 sub boxes -->
      <div class="sub-boxes hidden">
        <div class="sub-box">
          <div class="sub-header">
            <span class="sub-icon material-icons">architecture</span>
            <h4>Vendor Porting Kit</h4>
          </div>
          <p>Refer the vendor porting kit documentation to know how to port RDK in your platform.</p>
          <a href="../rdk7-vendor-porting-guide/" class="view-docs-btn">View Docs</a>
        </div>
        <div class="sub-box">
          <div class="sub-header">
            <span class="sub-icon material-icons">devices</span>
            <h4>HAL Specifications</h4>
          </div>
          <p>For details on HAL specification and HAL validation, please follow the below link</p>
          <a href="https://rdkcentral.github.io/rdk-hpk-documentation/" class="view-docs-btn">View Docs</a>
        </div>
        <div class="sub-box">
          <div class="sub-header">
            <span class="sub-icon material-icons">assignment</span>
            <h4>RDK Certification</h4>
          </div>
          <p>For details on the RDK Certification process and the certification suite coverage, please follow below link.</p>
          <a href="https://wiki.rdkcentral.com/pages/viewpage.action?pageId=131729119" class="view-docs-btn">View Docs</a>
        </div>
      </div>
      <span class="expandable-icon-bg material-icons"style="color:var(--rdk-green);">architecture</span>
    </div>
    </div>
    <!-- Box 4 -->
    <div class="expandable-wrap">
    <div class="expandable-box" data-box="4">
      <div class="box-main">
        <img src="../../../assets/icons/components (1).png" class="box-header-icon"> 
        <div class="box-header">
          <h3>Core Components</h3>
        </div>
        <p class="box-description">Explore the developer resources for detailed insights into the architecture and components</p>
        <span class="expand-icon material-icons" style="background-color:var(--rdk-yellow);">add</span>
      </div>
      <!-- Box 4 sub boxes -->
      <div class="sub-boxes hidden">
        <div class="sub-box">
          <div class="sub-header">
            <span class="sub-icon material-icons">layers</span>
            <h4>Architecture</h4>
          </div>
          <p>To know more about the layered structure and architecture of the RDK, please follow the below link</p>
          <a href="../rdk7-architecture/" class="view-docs-btn">View Docs</a>
        </div>
        <div class="sub-box">
        <div class="sub-header">
          <span class="sub-icon material-icons">build</span>
          <h4>Device Profiles</h4>
        </div>
        <p>To know more on the different device profiles supported by RDK, please follow below link.</p>
        <a href="../video_profiles/" class="view-docs-btn">View Docs</a>
      </div>
      <div class="sub-box">
        <div class="sub-header">
          <span class="sub-icon material-icons">assignment</span>
          <h4>RDK Components</h4>
        </div>
        <p>For detailed documentation of the individual RDK components, please follow the below link.</p>
        <a href="../rdkv_components/" class="view-docs-btn">View Docs</a>
      </div>
      <div class="sub-box">
        <div class="sub-header">
          <span class="sub-icon material-icons">assignment</span>
          <h4>Device Management</h4>
        </div>
        <p>To explore the remote device management capabilities available for RDK-E, please follow the below link.</p>
        <a href="../../../device-management/docs/" class="view-docs-btn">View Docs</a>
      </div>
    </div>
    <span class="expandable-icon-bg material-icons"style="color:var(--rdk-yellow);">layers</span>
    </div>
    </div>
    <!-- Box 5 -->
    <div class="expandable-wrap">
    <div class="expandable-box" data-box="5">
      <div class="box-main">
        <img src="../../../assets/icons/criteria.png" class="box-header-icon"> 
        <div class="box-header">
          <h3>Test & Certification</h3>
        </div>
        <p class="box-description">RDK provides a suite of in-house testing tools to validate your deployments and certify your RDK ports.</p>
        <span class="expand-icon material-icons" style="background-color:var(--rdk-orange);">add</span>
      </div>
      <!-- Box 5 sub boxes -->
      <div class="sub-boxes hidden">
        <div class="sub-box">
          <div class="sub-header">
            <span class="sub-icon material-icons">layers</span>
            <h4>Overview</h4>
          </div>
          <p>To get an overview of tests and certifications, please follow the below link.</p>
          <a href="#" class="view-docs-btn">View Docs</a>
        </div>
        <div class="sub-box">
          <div class="sub-header">
            <span class="sub-icon material-icons">layers</span>
            <h4>TDK</h4>
          </div>
          <p>Refer the TDK documentation to know the features, coverage and operational details.</p>
          <a href="../../../tools/docs/TDK/" class="view-docs-btn">View Docs</a>
        </div>
        <div class="sub-box">
        <div class="sub-header">
          <span class="sub-icon material-icons">build</span>
          <h4>Automatics</h4>
        </div>
        <p>To know the features, coverage and operational details of automatics, check the automatics documentation.</p>
        <a href="../../../tools/docs/Automatics/" class="view-docs-btn">View Docs</a>
      </div>
      <div class="sub-box">
        <div class="sub-header">
          <span class="sub-icon material-icons">assignment</span>
          <h4>RDK Certification</h4>
        </div>
        <p>For details on the RDK Certification process and the certification suite coverage, please follow below link.</p>
        <a href="https://wiki.rdkcentral.com/pages/viewpage.action?pageId=131729119" class="view-docs-btn">View Docs</a>
      </div>
    </div>
    <span class="expandable-icon-bg material-icons"style="color:var(--rdk-orange);">assignment</span>
    </div>
  </section>
</div>

<script>
//align subboxes child elements according to height of the header
function alignSubBoxParagraphs() {
  document.querySelectorAll('.sub-boxes').forEach(container => {
    const headers = [...container.querySelectorAll('.sub-header')];
    if (!headers.length) return;

    const maxHeight = Math.max(...headers.map(h => h.offsetHeight));
    headers.forEach(h => {
      const diff = maxHeight - h.offsetHeight;
      if (diff > 0) {
        h.style.marginBottom = `${diff + 8}px`; // adds space below header
      }
    });
  });
}

document.addEventListener('DOMContentLoaded', alignSubBoxParagraphs);
document.querySelectorAll('.expand-icon').forEach(icon =>
  icon.addEventListener('click', () => setTimeout(alignSubBoxParagraphs, 500))
);

(function () {

  // CSS selectors for icons, boxes, and grid container
  const ICON_SELECTOR = '.expand-icon';
  const BOX_SELECTOR = '.expandable-box';
  const WRAP_SELECTOR = '.expandable-wrap';
  const GRID_SELECTOR = '.expandable-section';

  // Store original order of grid items (wrappers or boxes depending on DOM)
  const grid = document.querySelector(GRID_SELECTOR);
  const originalOrder = grid
    ? Array.from(grid.children).filter(n => n.matches(`${WRAP_SELECTOR}, ${BOX_SELECTOR}`))
    : [];

  /*Function to find how many columns the grid has.*/
  function getColumnsCount(gridEl) {
    const style = window.getComputedStyle(gridEl);
    const cols = style.gridTemplateColumns;
    return cols ? cols.split(/\s+/).filter(Boolean).length : 4;
  }

  /* Helper: the element that is the actual grid item (direct child of grid) */
  function getGridItemFor(box) {
    const wrap = box.closest(WRAP_SELECTOR);
    // If wrapper exists and is a direct child of the grid, use it; otherwise fallback to box
    if (wrap && wrap.parentElement && wrap.parentElement.matches(GRID_SELECTOR)) {
      return wrap;
    }
    return box;
  }

  /* Move a grid item to the start of its row in the grid */
  function moveBoxToRowStart(gridItem) {
    const gridEl = gridItem.closest(GRID_SELECTOR);
    if (!gridEl) return;

    // Collect direct children (actual grid items)
    const items = Array.from(gridEl.children)
      .filter(n => n.matches(`${WRAP_SELECTOR}, ${BOX_SELECTOR}`));

    const rowsMap = new Map(); // groups items by row
    const TOL = 6; // px tolerance for detecting same row

    items.forEach(item => {
      const top = Math.round(item.getBoundingClientRect().top);
      let foundKey = null;

      for (let key of rowsMap.keys()) {
        if (Math.abs(key - top) <= TOL) { foundKey = key; break; }
      }

      const key = (foundKey !== null) ? foundKey : top;
      if (!rowsMap.has(key)) rowsMap.set(key, []);
      rowsMap.get(key).push(item);
    });

    // Find the row of the current grid item
    const itemTop = Math.round(gridItem.getBoundingClientRect().top);
    let itemRowKey = null;
    for (let key of rowsMap.keys()) {
      if (Math.abs(key - itemTop) <= TOL) { itemRowKey = key; break; }
    }
    if (itemRowKey === null) return;

    const rowItems = rowsMap.get(itemRowKey) || [];
    if (!rowItems.length) return;

    // Move item to the start of its row
    const firstInRow = rowItems[0];
    if (firstInRow !== gridItem) {
      gridEl.insertBefore(gridItem, firstInRow);
    }
  }

  /* Expand a box to full width and show its sub-boxes */
  function expandBox(box, icon) {
    const gridItem = getGridItemFor(box);
    moveBoxToRowStart(gridItem);

    box.classList.remove('collapsed');
    box.classList.add('expanded');

    // Span the actual grid item across all columns
    gridItem.style.gridColumn = '1 / -1';

    if (icon) icon.textContent = 'remove';

    const subBoxes = box.querySelector('.sub-boxes');
    if (subBoxes) {
      subBoxes.classList.remove('hidden');
      subBoxes.style.gridTemplateColumns = 'repeat(4, 1fr)';
    }
  }

  // Collapse a box back to normal size and hide its sub-boxes
  function collapseBox(box, icon) {
    const gridItem = getGridItemFor(box);
    box.classList.add('collapsed');

    // After animation, reset styles
    setTimeout(() => {
      box.classList.remove('expanded', 'collapsed');
      gridItem.style.gridColumn = '';

      // Restore original order of grid items
      if (grid && originalOrder.length) {
        originalOrder.forEach(item => grid.appendChild(item));
      }
    }, 400);

    if (icon) icon.textContent = 'add';

    const subBoxes = box.querySelector('.sub-boxes');
    if (subBoxes) subBoxes.classList.add('hidden');
  }

  // Attach click handlers to all expand icons
  document.querySelectorAll(ICON_SELECTOR).forEach(icon => {
    icon.addEventListener('click', () => {
      const box = icon.closest(BOX_SELECTOR);
      if (!box) return;

      if (box.classList.contains('expanded')) {
        collapseBox(box, icon);
      } else {
        expandBox(box, icon);
      }
    });
  });

  // Handle window resize: re-align expanded boxes
  let resizeTimer = null;
  window.addEventListener('resize', () => {
    clearTimeout(resizeTimer);
    resizeTimer = setTimeout(() => {
      document.querySelectorAll(`${GRID_SELECTOR} ${BOX_SELECTOR}.expanded`).forEach(box => {
        const gridItem = getGridItemFor(box);
        moveBoxToRowStart(gridItem);
        gridItem.style.gridColumn = '1 / -1';
      });
    }, 120);
  });
})();

// Equalize row heights in the grid
function equalizeRowHeights() {
  const grid = document.querySelector('.expandable-section');
  if (!grid) return;

  // Use direct grid children to group rows
  const gridItems = Array.from(grid.children)
    .filter(n => n.matches('.expandable-wrap, .expandable-box'));

  const rowsMap = new Map();
  const TOL = 6;

  // Reset inner box min-heights before measuring
  gridItems.forEach(item => {
    const innerBox = item.matches('.expandable-box')
      ? item
      : item.querySelector('.expandable-box');
    if (innerBox) innerBox.style.minHeight = 'auto';
  });

  // Group by row position (top)
  gridItems.forEach(item => {
    const top = Math.round(item.getBoundingClientRect().top);
    let foundKey = null;

    for (let key of rowsMap.keys()) {
      if (Math.abs(key - top) <= TOL) { foundKey = key; break; }
    }

    const key = (foundKey !== null) ? foundKey : top;
    if (!rowsMap.has(key)) rowsMap.set(key, []);
    rowsMap.get(key).push(item);
  });

  // Equalize the inner .expandable-box heights within each row
  rowsMap.forEach(rowItems => {
    const innerBoxes = rowItems.map(item =>
      item.matches('.expandable-box') ? item : item.querySelector('.expandable-box')
    ).filter(Boolean);

    if (!innerBoxes.length) return;

    const maxHeight = Math.max(...innerBoxes.map(b => b.offsetHeight));
    innerBoxes.forEach(b => b.style.minHeight = `${maxHeight}px`);
  });
}
</script>
